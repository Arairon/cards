...
class Client:
    ...
    def gameRun(self, txt, target = None):
        if target is None:
            for i in self.players:
                if i.active:
                    #print(f"{self.username} >> Passing {txt} to {i.username}")
                    log.log(f"{self.username} >> Passing {txt} to {i.username}", 'gameRun()', 'GRun')
                    i.s.send(txt.encode('utf-8'))
                    if txt == '%online()':
                        log.log(f"{self.username} started a game with {self.players[-1].username}\n", 'gameRun()', 'GameStart')
        else:

            for i in self.players:
                print(f'{(int((i.username.split("-")[-1].split("@")[0])))}  - {int(target)}')
                if i.active and (int((i.username.split("-")[-1].split("@")[0])) == int(target)):
                    log.log(f"{self.username} >> Passing {txt} to {i.username}", 'gameRun()', 'GRun')
                    i.s.send(txt.encode('utf-8'))
                    if txt == '%online()':
                        log.log(f"{self.username} started a game with {self.players[-1].username}\n", 'gameRun()',
...
def accept():
    ...
    while True:
        try:
            ...
            #print(f'{e}\n@accept')
            log.log(f'Error: {e} -> Dropping', 'accept', 'ERROR')
            client.send('>An error was caused, please try to reconnect\n'.encode('utf-8'))
            break
            ...
            if msg[0] not in '/%$' and msg != '':
                ...
                if msg.split()[0] == "/leave":
                    ...
                    try:
                        ...
                        client.s.send(f"Usage: /connect [clientID]".encode("utf-8"))
                ...
                elif msg.split()[0] == "/getID":
                    log.log(f'Sending %recvID({client.username.split("-")[-1].split("@")[0]}) to {client.name}', '/getID')
                    client.s.send(f"%recvID({client.username.split('-')[-1].split('@')[0]})".encode('utf-8'))
                    ...
                    if msg.split()[0][1] == '>':
                        toSend = msg.split()[0].split('>')
                        recvId = toSend[1]
                        toSend = ''.join([toSend[0], toSend[2]])
                        client.gameRun(toSend, recvId)
                        pass
                    else:
                        client.gameRun(msg)
